...
const char * const BITCOIN_CONF_FILENAME = "selacoin.conf";
const char * const BITCOIN_PID_FILENAME = "selacoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "selacoin" is a composite category enabling all SelaCoin-related debug output
                if(ptrCategory->count(std::string("selacoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SelaCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SelaCoin
    // Mac: ~/Library/Application Support/SelaCoin
    // Unix: ~/.selacoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SelaCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SelaCoin";
#else
    // Unix
    return pathRet / ".selacoin";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty selacoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override selacoin.conf
