...
# selacoin_hash
RUN git clone https://github.com/selacoinpay/selacoin_hash
RUN cd selacoin_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} selacoin
RUN useradd -u ${USER_ID} -g selacoin -s /bin/bash -m -d /selacoin selacoin
...
RUN mkdir /selacoin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /selacoin-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /selacoin-src
