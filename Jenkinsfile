...
// which are part of the SelaCoin repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="selacoin-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /selacoin-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "selacoinpay-selacoin/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from selacoinpay-selacoin/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'selacoinpay-selacoin/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from selacoinpay-selacoin/develop"
            }
          }

          if (hasCache) {
            sh "cd /selacoin-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /selacoin-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /selacoin-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /selacoin-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /selacoin-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /selacoin-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /selacoin-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
